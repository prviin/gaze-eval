...
RESULTS_DIR = Path("data/processed/ueyes/results")
OUTPUT_DIR = Path("data/processed/ueyes/paper_tables")

SUMMARY_PATH = RESULTS_DIR / "ueyes_pair_metric_summary_seed42.csv"
SANITY_PATH = RESULTS_DIR / "ueyes_pair_metric_sanity_seed42.csv"
DIFFERENCES_PATH = RESULTS_DIR / "ueyes_pair_metric_differences_seed42.csv"
BASELINE_PATH = RESULTS_DIR / "ueyes_human_baseline_quality_seed42.csv"
...
def main() -> None:
    ...
    main_table_path = OUTPUT_DIR / "ueyes_main_metric_table.csv"
    baseline_table_path = OUTPUT_DIR / "ueyes_human_baseline_table.csv"
    appendix_table_path = OUTPUT_DIR / "ueyes_metric_summary_appendix.csv"
    latex_table_path = OUTPUT_DIR / "ueyes_main_metric_table.tex"
...
def make_main_paper_table(
    *,
    ...
    table = round_numeric_columns(table)
...
def dataframe_to_latex_table(table: pd.DataFrame) -> str:
    ...
    return latex_table.to_latex(
        ...
        longtable=False,
        caption=(
            "UEyes human-human scanpath similarity benchmark. "
            "The reported difference compares same-image pairs against "
            "different-category pairs and is oriented so that positive values "
            "indicate better same-image similarity."
        ),
        label="tab:ueyes-human-similarity",
    )
